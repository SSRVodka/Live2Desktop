...
#include "modules/stt/common/json.hpp"
#include "modules/stt/common/common.h"
...
void STT::Server::whisper_print_usage(int /*argc*/, char ** argv, const whisper_params & params, const server_params& sparams) {
    fprintf(stderr, "\n");
...
bool STT::Server::whisper_params_parse(int argc, char ** argv, whisper_params & params, server_params & sparams) {
...
void STT::Server::check_ffmpeg_availibility() {
...
std::string STT::Server::generate_temp_filename(const std::string &prefix, const std::string &extension) {
...
bool STT::Server::convert_to_wav(const std::string & temp_filename, std::string & error_resp) {
...
std::string STT::Server::estimate_diarization_speaker(std::vector<std::vector<float>> pcmf32s, int64_t t0, int64_t t1, bool id_only) {
    std::string speaker = "";
...
void STT::Server::whisper_print_progress_callback(struct whisper_context * /*ctx*/, struct whisper_state * /*state*/, int progress, void * user_data) {
...
void STT::Server::whisper_print_segment_callback(struct whisper_context * ctx, struct whisper_state * /*state*/, int n_new, void * user_data) {
...
std::string STT::Server::output_str(struct whisper_context * ctx, const whisper_params & params, std::vector<std::vector<float>> pcmf32s) {
...
void STT::Server::get_req_parameters(const httplib::Request & req, whisper_params & params) {
...
int STT::Server::cli_main(int argc, char *argv[]) {
    ...
    svr.Get(sparams.request_path + "/", [&default_content](const httplib::Request &, httplib::Response &res){
        res.set_content(default_content, "text/html");
        return false;
    });

    svr.Options(sparams.request_path + sparams.inference_path, [&](const httplib::Request &, httplib::Response &){
    });

    svr.Post(sparams.request_path + sparams.inference_path, [&](const httplib::Request &req, httplib::Response &res){
        ...
        // run the inference
        {
            ...
            wparams.token_timestamps = !params.no_timestamps && params.response_format == OutputType::vjson_format;
        ...
        if (params.response_format == OutputType::text_format)
        {
            std::string results = output_str(ctx, params, pcmf32s);
            res.set_content(results.c_str(), "text/html; charset=utf-8");
        }
        else if (params.response_format == OutputType::srt_format)
        {
            std::stringstream ss;
            const int n_segments = whisper_full_n_segments(ctx);
            for (int i = 0; i < n_segments; ++i) {
        ...
        } else if (params.response_format == OutputType::vtt_format) {
            std::stringstream ss;
        ...
        } else if (params.response_format == OutputType::vjson_format) {
    ...
    svr.Post(sparams.request_path + "/load", [&](const httplib::Request &req, httplib::Response &res){
        std::lock_guard<std::mutex> lock(whisper_mutex);
